...
object kMeansClustering {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val testImages = sc.wholeTextFiles(s"data/output/mainframes/*.jpg")
    val testImagesArray = testImages.collect()

    val Array(training, test) = testImages.randomSplit(Array(0.6, 0.4))

    val model = NaiveBayes.train()

    //Look at how training data is!
    // testImagesArray.foreach(f=>println(f))
